...
Every stateful workload here is crash-safe by design — etcd and Ceph journal, Postgres and MySQL replay their logs — so an abrupt power-off costs boot time, not data. MySQL is the one that needs help coming back, and both halves of that live in `gitops/workloads/raw/mysql/manifests/`. `10-innodbcluster.yaml` keeps the routers out of the way: each polls `replication_group_members` for an ONLINE member before bootstrapping instead of racing the group's reboot-from-complete-outage, and there are two of them. `20-outage-reboot.yaml` performs the reboot, because the operator will not — roughly every second power-off leaves all three members up and healthy with Group Replication down and the `InnoDBCluster` stuck at `OFFLINE` / `ONLINE 0`, and the operator's own repair path is unreachable from that state (the manifest's header comment has the details). A CronJob checks every five minutes, requires all three members to report zero ONLINE members twice sixty seconds apart, and then runs `dba.rebootClusterFromCompleteOutage()`. Expect recovery within ~6 minutes of the members coming up; `kubectl -n mysql logs job/<name>` shows what it did.
