...
- **Short commit messages.** Title + 3-6 line body. Analysis, rationale, and test plan go in the PR description, not the commit.
...
4. **Branch → commit → PR** — create a new branch, commit (short message, see Rules), push, and open a PR with the analysis and rationale in the description.
5. **Re-enable Argo CD sync** — note this in the **PR description only** (not the commit message) so the developer restores automated sync after merge.
